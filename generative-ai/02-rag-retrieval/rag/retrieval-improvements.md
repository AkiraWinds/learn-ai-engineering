...
| kb_adk + thinking1024 | 0.583 | 0.531 | 7.3s |
| kb_lg + CRAG | 0.547 | 0.531 | 7.7s |
| kb_lg no CRAG | 0.542 | 0.531 | 4.4s ← best latency/quality tradeoff |
| kb_adk baseline | 0.418 | 0.219 | 2.6s |

Gap to va_staging: **-0.042 MRR** (best local vs benchmark on smoke subset).

## What we learned from ablations

- **Thinking budget (+1024)**: huge gain for kb_adk (+0.165 MRR), hurts kb_lg when combined with CRAG (interaction effect — thinking changes citation style, breaking CRAG's grading alignment)
...
**How**: `GEMINI_MODEL=gemini-3-flash-preview uv run uvicorn kb_adk.main:app --port 8011`
...
**Implementation in kb_lg**: New node `multi_query` between `planner` and `retrieve`.
...
**Implementation in kb_adk**: Modify `fetch_support_knowledge` tool to accept and execute multiple queries.
...
**Limitation**: Works best with pure vector retrieval (kb_rag). Bedrock HYBRID search (BM25 + vector) may dilute the benefit since BM25 is keyword-based.

**Best implementation path**: kb_rag's retrieval endpoint, not kb_adk/kb_lg with Bedrock.
...
**Status**: PLANNED — implement as `RETRIEVAL_MODE=hyde` flag on kb_rag after multi-query.

---

### 4. kb_rag as retrieval backend for kb_adk / kb_lg

**What**: Route all Bedrock KB calls through kb_rag's local DuckDB+multilingual-e5-large endpoint.
**Why**: Zero Bedrock cost, full control over embedding model, enables HyDE.
**Flags**: `SUPPORT_AGENT_RETRIEVAL_MODE=rag` (kb_adk), `SUPPORT_AGENT_RETRIEVAL_BACKEND=rag` (kb_lg)
**Baseline**: kb_rag alone scores MRR=0.42 on retrieval-only. LLM agents + local rag TBD.
...
2. For hit tasks: va_staging / kb_lg+thinking responses as preferred, adk_baseline as rejected
...
2. **kb_rag as backend** (free latency/cost benchmark)
3. **Multi-query retrieval** (best quality lever after model upgrade)
4. **HyDE on kb_rag** (complements multi-query, different failure mode coverage)
5. **DPO** (long-term, blocked on data collection)

---

## kb_rag baseline (smoke, 2026-05-09)
...
3/9 annotated tasks hit in top-5. **2 misses are ProductWiki URLs not ingested — this is a coverage gap, not a ranking error.** When filtering those, effective MRR on ingested content is higher.

This eval loop uses a different retrieval path than the citation proxy: `query → POST /api/v1/retrieval → ranked chunks → compare URLs to golden set → P@k / MRR / NDCG`. The existing retrieval proxy measures citation sentiment on support corpus A conversation data; it can't measure ranked chunk quality.

### URL normalization

**Design decision:** `kb_url_map.json` is a stateless lookup table committed to the repo. Agents never contain version-aware URL logic — KB migrations (Product → Partner) are a data operation, not a code redeploy. This keeps agents KB-agnostic and prevents grounding false-positives when the underlying domain changes.

Match on path-only (`/da/articles/622808-...`) to avoid protocol/domain drift between `help.billy.dk` and `help.shine.co`.

**Aliasing rules:**
1. SupportPlatform articles: extract numeric ID from `/articles?/(\d+)` — locale-agnostic.
...
3. ProductWiki (`www.billy.dk/billypedia/<slug>/`): **no canonical Partner URL**. These are excluded from grounding/citation coverage (implemented in `url_coverage.py` as `topic_proxy`). Do NOT alias; tag `content_type: "glossary_term"` if ever ingested.

### ProductWiki: never the primary retrieval target

ProductWiki is a flat concept glossary with no numeric article IDs and no Partner equivalent. For `definition` queries it provides a context frame only — the actual answer still comes from help center articles. This is why ProductWiki misses in eval results are **expected and correct** (not ranking failures). The source router in `semantic-cache.md` encodes this: `definition` intent → ProductWiki as context enrichment + help center for the answer.
